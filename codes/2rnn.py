...
def prepare_sequence(premise, hypothesis):
    ...
    prem_idx = torch.tensor([vocab_dict[w] for w in prem_list], dtype=torch.long, device=device)
    hypo_idx = torch.tensor([vocab_dict[w] for w in hypo_list], dtype=torch.long, device=device)
    return {'prem_idx': prem_idx, 'hypo_idx':hypo_idx}
...
class DQN(nn.Module):
    ...
    def forward(self, state):
        prem_idx = state['prem_idx']
        hypo_idx = state['hypo_idx']
        prem_embeds = self.word_embeddings(prem_idx)
